...
- Target每次按单格宽度在MissArea内随机对齐；括约肌不改变QTE出现频率、指针速度、透明度或所需连续成功次数。
...
- QTE使用Pixel Art拼接视觉：Frame只作装饰，MissArea是指针移动边界，Target由Left、3/5/7个独立Center和Right无缝组成，Center不会横向拉伸。
- Pointer完整保持在MissArea左右边界内往返；按空格时以Pointer中心是否位于完整Target水平范围内判定成功或失败。
- QTE 只在厕所回合开始后调度。
...
独立测试场景为 `res://scenes/tests/qte_test.tscn`，可选择HIGH、MID、LOW并分别验证3、5、7个Center格；选择只影响下一次启动的QTE。
