...
主要文件：`res://scripts/player_stats.gd`。
...
`SmoothnessBar` 和 `SmoothnessLabel` 读取 `PlayerStats.smoothness`：
...
- 夹断次数不参与价格公式。
- `Economy.total_money` 作为 Autoload 跨场景保留，关闭游戏后重置。
...
部分实现：`ClogTargetLabel` 仍是占位文字；当前阈值超出量使用 `BottomMarker`，不是 `TopMarker`。
