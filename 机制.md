...
8. 统一结算最终长度、堵塞进度和金钱。
9. 任意一天达到当天堵塞目标都会显示成功结果；第 1～4 天未达标时，结算后进入End Day当日结果页，点击`Wipe your a**`进入商店，再推进到堵塞进度为0的下一天。
...
- Tube下方按本轮所有固定断段长度与当前active段实际长度的总和显示剩余距离；目标厘米数由`TARGET_CLOG_PROGRESS / CLOG_PROGRESS_PER_CM`实时派生，再向上取整为`nFT TO CLOG`。active段缩回会让剩余距离重新增加，实际总长度达到目标时显示`0FT TO CLOG`。
...
- QTE激活期间Tube流动动画也停在当前帧；判定结束后重新检查真实管道状态，仅在仍有Poop时继续播放。
...
- Distance till clogging使用`max(堵塞目标 - 当天最终进度, 0) / 每厘米堵塞点数`换算为厘米，再向下转换为整数FT；三段距离条按当天最终进度比例显示，不参与玩法计算。
...
| `GameState` | 当前天数、当天堵塞进度、上一轮只读结算快照 | 恢复第1天、当天堵塞归零、清空结算快照，并协调其他系统重置与每日状态页切换 |
...
- 正式流程场景：`res://scenes/StartingPage.tscn`、`res://scenes/IntroStory.tscn`、`res://day_start.tscn`、`res://scenes/toilet.tscn`、`res://scenes/end_day.tscn` 与 `res://scenes/shop.tscn`。
