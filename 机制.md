...
本文档根据当前场景和 GDScript 整理。项目主场景为 `res://scenes/StartingPage.tscn`，点击START后进入 `res://scenes/toilet.tscn`。

## 1. 核心玩法循环

1. 启动项目后显示StartingPage；只有点击START按钮才进入厕所。
2. 进入厕所后生成 Poop，但倒计时暂不启动。
3. 第一次有效按下鼠标左键后，10 秒回合与 QTE 调度同时开始。
4. 玩家通过单击或长按延长当前 Poop 段；QTE失败会夹断并立即开始新段。
5. 倒计时归零后停止新输入与 QTE，等待活动段完成推出和所有断段完成下落。
6. 统一结算最终长度、堵塞进度和金钱。
7. 第 1～4 天可进入商店，再点击 `NEXT DAY` 开始下一天。
8. 第 5 天直接显示成功或失败结果。
...
- 重新开始会解除暂停并重新加载厕所；新厕所的10秒Timer仍保持停止，直到第一次有效左键输入。
...
| `GameState` | 当前天数、跨天堵塞进度 | 恢复第1天、堵塞归零，并协调其他系统重置与厕所场景切换 |
...
```mermaid
flowchart TD
    S[StartingPage] -->|点击START| A[厕所：等待首次有效左键]
    A --> B[10秒厕所回合]
    B --> C[停止输入和QTE]
    C --> D[等待活动段完成移动和断段落定]
    D --> E[结算长度、堵塞和金钱]
    E -->|第1～4天| F[商店]
    F --> H[购买器官或排队食物]
    H -->|NEXT DAY：激活食物并推进天数| A
    E -->|第5天| G[成功或失败结果]
    A -. MENU或Esc .-> P[暂停菜单]
    B -. MENU或Esc .-> P
    F -. MENU或Esc .-> P
    P -->|RESUME| A
    P -->|RESTART RUN确认| R[重置本局并加载第1天厕所]
    R --> A
```
...
- 正式流程场景：`res://scenes/StartingPage.tscn`、`res://scenes/toilet.tscn` 与 `res://scenes/shop.tscn`。
- StartingPage等待现有START按钮的 `pressed` 信号；不会在 `_ready()` 中自动跳转。
