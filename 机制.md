...
- 食物在商店购买时立即通过 `Economy`扣钱。同一商店阶段可购买任意多种不同食物，但每个食物ID最多购买一份；购买资格只检查余额和当前pending列表是否已有同一ID，不存在总数量或槽位限制。
...
- 新食物购买按钮与排队预览UI尚未接入，因此目前无法从商店画面购买食物；`FoodSystem`、`FoodDefinitions`、pending/active数据、每种食物限购一次及次日生效逻辑仍然保留。
